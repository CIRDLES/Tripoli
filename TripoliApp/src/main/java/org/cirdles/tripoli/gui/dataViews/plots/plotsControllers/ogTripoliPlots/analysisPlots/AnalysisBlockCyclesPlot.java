...
public class AnalysisBlockCyclesPlot extends AbstractPlot {
    ...
    private AnalysisBlockCyclesPlot(Rectangle bounds, AnalysisBlockCyclesRecord analysisBlockCyclesRecord, PlotWallPane parentWallPane) {
        ...
        this.ratio = analysisBlockCyclesRecord.isRatio();
        this.blockMode = true;
    }
    ...
    public void plotStats(GraphicsContext g2d) {
        ...
        if (blockMode) {
            ...
            for (int blockIndex = 0; blockIndex < blockStatsRecords.length; blockIndex++) {
                int cyclesPerBlock = blockStatsRecords[blockIndex].cyclesCount();
                ...
                double standardDeviation = blockStatsRecords[blockIndex].standardDeviation();
                double standardError = blockStatsRecords[blockIndex].standardError();

                double plottedTwoSigmaHeight = Math.min(mapY(mean - 2.0 * standardDeviation), topMargin + plotHeight) - Math.max(mapY(mean + 2.0 * standardDeviation), topMargin);
                g2d.setFill(OGTRIPOLI_TWOSIGMA);
                g2d.fillRect(leftX, Math.max(mapY(mean + 2.0 * standardDeviation), topMargin), rightX - leftX, plottedTwoSigmaHeight);

                double plottedOneSigmaHeight = Math.min(mapY(mean - standardDeviation), topMargin + plotHeight) - Math.max(mapY(mean + standardDeviation), topMargin);
                g2d.setFill(OGTRIPOLI_ONESIGMA);
                g2d.fillRect(leftX, Math.max(mapY(mean + standardDeviation), topMargin), rightX - leftX, plottedOneSigmaHeight);

                double plottedTwoStdErrHeight = Math.min(mapY(mean - 2.0 * standardError), topMargin + plotHeight) - Math.max(mapY(mean + 2.0 * standardError), topMargin);
                g2d.setFill(OGTRIPOLI_TWOSTDERR);
                g2d.fillRect(leftX, Math.max(mapY(mean + 2.0 * standardError), topMargin), rightX - leftX, plottedTwoStdErrHeight);
        ...
        for (Map.Entry<Integer, BlockCyclesRecord> entry : mapBlockIdToBlockCyclesRecord.entrySet()) {
            ...
            if (blockCyclesRecord != null) {
                blockStatsRecords[arrayIndex] = BlockStatsRecord.generateBlockStatsRecord(
                        entry.getKey(), ratio, blockCyclesRecord.cycleMeansData(), blockCyclesRecord.cyclesIncluded().length);
            }
            arrayIndex++;
        }
